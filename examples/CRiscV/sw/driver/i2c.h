...
static void i2c_applyConfig(u32 reg, I2c_Config *config) {
    write32(reg + I2C_SAMPLING_CLOCK_DIVIDER, config->samplingClockDivider);
    write32(reg + I2C_TIMEOUT, config->timeout);
    write32(reg + I2C_TSUDAT, config->tsuDat);

    write32(reg + I2C_TLOW, config->tLow);
    write32(reg + I2C_THIGH, config->tHigh);
    write32(reg + I2C_TBUF, config->tBuf);
}

static inline void i2c_filterEnable(u32 reg, u32 filterId, u32 config){
    write32(reg + I2C_FILTERING_CONFIG + 4*filterId, config);
}



static inline void i2c_masterStart(u32 reg){
    write32(reg + I2C_MASTER_STATUS, I2C_MASTER_START);
}


static int i2c_masterBusy(u32 reg){
    return (read32(reg + I2C_MASTER_STATUS) & I2C_MASTER_BUSY) != 0;
}
...
static inline void i2c_masterStop(u32 reg){
    write32(reg + I2C_MASTER_STATUS, I2C_MASTER_STOP);
}
...
static inline void i2c_masterDrop(u32 reg){
    write32(reg + I2C_MASTER_STATUS, I2C_MASTER_DROP);
}
...
static inline void i2c_listenAck(u32 reg){
    write32(I2C_RX_LISTEN ,reg + I2C_RX_ACK);
}

static inline void i2c_txByte(u32 reg,u8 byte){
    write32(reg + I2C_TX_DATA, byte | I2C_TX_VALID | I2C_TX_ENABLE | I2C_TX_DISABLE_ON_DATA_CONFLICT);
}


static inline void i2c_txAck(u32 reg){
    write32(reg + I2C_TX_ACK, I2C_TX_VALID | I2C_TX_ENABLE);
}
static inline void i2c_txNack(u32 reg){
    write32(reg + I2C_TX_ACK, 1 | I2C_TX_VALID | I2C_TX_ENABLE);
}
static void i2c_txAckWait(u32 reg){
    while(read32(reg + I2C_TX_ACK) & I2C_TX_VALID);
}
...
static u32 i2c_rxData(u32 reg){
    return read32(reg + I2C_RX_DATA) & I2C_RX_VALUE;
}

static int i2c_rxNack(u32 reg){
    return (read32(reg + I2C_RX_ACK) & I2C_RX_VALUE) != 0;
}
...
static void i2c_txByteRepeat(u32 reg,u8 byte){
    write32(reg + I2C_TX_DATA, byte | I2C_TX_VALID | I2C_TX_ENABLE | I2C_TX_DISABLE_ON_DATA_CONFLICT | I2C_TX_REPEAT);
}

static void i2c_txNackRepeat(u32 reg){
    write32(reg + I2C_TX_ACK, 1 | I2C_TX_VALID | I2C_TX_ENABLE | I2C_TX_REPEAT);
}

static inline void i2c_setFilterConfig(u32 reg, u32 filterId, u32 value){
    write32(reg + I2C_FILTERING_CONFIG + 4*filterId, value);
}

static void i2c_enableInterrupt(u32 reg, u32 value){
    write32(reg + I2C_INTERRUPT_ENABLE, value | read32(reg + I2C_INTERRUPT_ENABLE));
}
...
static inline void i2c_clearInterruptFlag(u32 reg, u32 value){
    write32(reg + I2C_INTERRUPT_FLAG, value);
}
...
static void i2c_write(u8 addr, u32 reg, u32 value) {
	i2c_masterStartBlocking(I2C0_BASE);
	i2c_txByte(I2C0_BASE, addr);
	i2c_txNackBlocking(I2C0_BASE);
	i2c_txByte(I2C0_BASE, reg);
	i2c_txNackBlocking(I2C0_BASE);

	i2c_txByte(I2C0_BASE, value);
	i2c_txAckBlocking(I2C0_BASE);
	i2c_masterStopBlocking(I2C0_BASE);
}

static u32 i2c_read(u8 addr, u32 reg) {
	i2c_masterStartBlocking(I2C0_BASE);
	i2c_txByte(I2C0_BASE, addr);
	i2c_txNackBlocking(I2C0_BASE);
	i2c_txByte(I2C0_BASE, reg);
	...
	u32 data = i2c_rxData(I2C0_BASE);
	i2c_rxAck(I2C0_BASE);
	i2c_masterStopBlocking(I2C0_BASE);
	return data;
}

static void i2c_write_array(u8 addr, u8 *msg, u8 size)
{
	i2c_masterStartBlocking(I2C0_BASE);
	i2c_txByte(I2C0_BASE, addr);
	i2c_txNackBlocking(I2C0_BASE);
